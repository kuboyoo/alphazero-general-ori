...
def review(canonical_board, game, curPlayer, net, mcts, i, temp=0, is_final=False):
  
  valids = game.getValidMoves(canonical_board, 0)
  _, value = net.predict(canonical_board, valids) #形勢評価値

  if is_final: #最終ターンは推論できない
    return value[curPlayer], None
...
def main(args):
  num_players = 2
  model_name = "./Heian-kyo/genbu.pt" #暫定最強
  ...
  for i, path in enumerate(pkl_paths):
    ...
    is_final = (score1 >= 15 or score2 >= 15 and i % num_players == 0)
    value, entropy = review(canonical_board, game, curPlayer, net, mcts, i+1, temp=1, is_final=is_final)
